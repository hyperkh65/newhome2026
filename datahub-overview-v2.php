...
function normalizeLiveOverviewSuppliers(array $rows): array {
  $normalized = [];
  foreach ($rows as $row) {
    $normalized[] = [
      'name' => trim((string)($row['corpNm'] ?? $row['name'] ?? '')),
      'region' => trim((string)($row['rgnNm'] ?? $row['region'] ?? $row['adrs'] ?? '')),
      'score' => !empty($row['score'])
        ? (int)$row['score']
        : min(
          95,
          60
          + (!empty($row['telNo']) ? 4 : 0)
          + (!empty($row['hmpgAdrs']) ? 4 : 0)
          + (!empty($row['ceoNm']) ? 2 : 0)
          + (!empty($row['adrs']) ? 2 : 0)
        ),
      'created_at' => trim((string)($row['rgstDt'] ?? $row['created_at'] ?? gmdate('c'))),
    ];
  }
  return array_values(array_filter($normalized, fn($row) => trim((string)$row['name']) !== ''));
}

function liveOverviewSnapshot(): ?array {
  $suppliers = datahub_cache_load('live_suppliers_365_80_100', 60 * 60 * 24 * 30);
  $supplierItems = normalizeLiveOverviewSuppliers($suppliers['items'] ?? []);
